  ...
  head: () => ({
    meta: [
      { title: "Quill — AI Email Generator for Formal Correspondence" },
      {
        name: "description",
        content:
          "Compose job applications, apologies, meeting requests, thank-yous and business inquiries with AI. Pick a tone, length and prompt, and get a polished draft.",
      },
      { property: "og:title", content: "Quill — AI Email Generator" },
      {
        property: "og:description",
        content: "Draft formal, well-structured emails in seconds with a prompt library for five email types.",
      },
    ],
  }),
  component: Index,
...
function Index() {
  ...
  const mutation = useMutation({
    mutationFn: async (state: FormState) => {
      const typeLabel = EMAIL_TYPES.find((t) => t.id === state.emailType)?.label ?? state.emailType;
      return callGenerate({ data: { ...state, emailType: typeLabel } });
    },
  ...
  const submit = () => {
    if (!form.purpose.trim()) {
      toast.error("Tell us the purpose of the email first.");
      return;
    }
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-6xl px-5 py-10 sm:px-8 lg:py-16">
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-xl text-sm leading-relaxed text-muted-foreground">
              A drafting desk for letters that matter. Choose the kind of email, set the tone, and receive a
              composed, ready-to-send draft.
            </p>
          </div>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-2">
          ...
          <div className="mt-6 space-y-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Recipient name" htmlFor="recipientName">
                <Input
                  ...
                  placeholder="Ms. Adebayo"
                  className="rounded-sm"
                />
              </Field>
              <Field label="Recipient role / company" htmlFor="recipientRole">
                <Input
                  id="recipientRole"
                  value={form.recipientRole}
                  onChange={(e) => set("recipientRole", e.target.value)}
                  placeholder="Hiring Manager, Northwind"
                  className="rounded-sm"
                />
              </Field>
            </div>
            ...
            <Field label="Key details" htmlFor="details">
              <Textarea
                ...
                placeholder="Dates, names, numbers, attachments, anything that must appear."
                rows={4}
                className="rounded-sm"
              />
            </Field>
        ...
        <section className="letterpress flex min-h-[28rem] flex-col rounded-sm p-6 sm:p-8">
          <div className="flex items-center justify-between border-b border-border pb-4">
            <h2 className="font-display text-xl font-semibold">Draft</h2>
            {draft && (
            ...
            {mutation.isPending && !draft ? (
              ...
              <div className="flex h-full flex-col items-center justify-center text-center">
                ...
                <p className="mt-4 max-w-xs text-sm text-muted-foreground">
                  Your composed letter will appear here, ready to copy into your mail client.
                </p>
              </div>
            )}
          </div>
      ...
      <PromptLibraryDialog open={libraryOpen} onOpenChange={setLibraryOpen} onApply={applyPrompt} />
    </main>
  );
}
